...
namespace DestructionProfiles
{
	 ...
	 * table already names.
	 *
	 * Compressive 30 MPa — EN 1992-1-1 Table 3.1, f_ck, the characteristic cylinder
	 * strength that gives C30/37 its name. (The 37 is the equivalent cube strength.)
	 *
	 * Shear 6 MPa — concrete has no single published shear strength, so this is the
	 * widely used 0.2 * f_ck; push-off tests on 30 MPa concrete report 6-8 MPa.
	 *
	 * Tensile 3 MPa — EN 1992-1-1 Table 3.1 gives f_ctm = 2.9 MPa for C30/37,
	 * ROUNDED UP TO 3.0 as the baseline anchor. The rounding is inside the material's
	 * own scatter, not a fudge: the same table's characteristic fractiles for C30/37
	 * are f_ctk,0.05 = 2.0 and f_ctk,0.95 = 3.8, so 3.0 sits comfortably within the
	 * band, and every document that names this baseline says "~3 MPa".
	 *
	 ...
	 * facing brick and the one Eurocode 6 worked examples use for f_b. Everything
	 * masonry-related keys off it: GeneralPurposeMortar's shear ceiling is 0.065 x
	 * this.
	 *
	 ...
	 * of those axes — 10 against 20, and 0.1 against 2 — which is DESIGN.md §2's
