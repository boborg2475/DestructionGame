 ...
 * =====================================================================================
 *
 * `FStructure::ReseatSpannedGroups` records a spanned opening as an `FSpannedArch`, and the axis
 * it hands `ApplyArchingThrust` for the sideways push is `Arch.TowardEndZero = TowardAbutmentCm[0]`
 * (Core/Structure.cpp) — the RAW vector from the spanned group's centre to the FIRST abutment's
 * centre of mass. For a normal in-plane opening (abutments left and right at the SAME out-of-plane
 * coordinate, symmetric about the group centre) that vector is purely in the wall plane and fine.
 *
 * But when a spanned group re-seats onto PERPENDICULAR walls — the corner-hung case, e.g. the
 * realistic shed's back wall arching into its two side walls after a deep band is removed — the
 * first abutment sits diagonally toward one corner, INBOARD of the wall plane. So
 * `TowardAbutmentCm[0]` carries an OUT-OF-PLANE component, and `ApplyArchingThrust` pushes each
 * springing along that diagonal axis. The springing bed joints then read a spurious lateral force
 * perpendicular to the wall — a net sideways push a planar arch's thrust line can never produce.
 * (The shed probe measured about +/-30,732 uu of it in Y.)
 *
 * The two END CENTRES are already computed a few lines down (`EndCentreCm[0]`/`[1]`, the mean
 * abutment CoM per end, used for `Arch.SpanCm`). Their out-of-plane components CANCEL between the
 * two ends — both ends of a back-wall span sit at the same back-wall Y — so the correct thrust
 * axis is `normalize(project_to_horizontal_seat_plane(EndCentreCm[0] - EndCentreCm[1]))`, which is
 * purely along the wall run. THIS TEST DOES NOT WRITE THAT FIX; it pins the behaviour it must
 * produce: a corner-hung springing carries NO out-of-plane force.
 *
 * =====================================================================================
 * THE FIXTURE — A SMALL, PURPOSE-BUILT CORNER-HUNG ARCH (world-free)
 * =====================================================================================
 *
 * A single back-wall brick S spans a hole (no bed joint beneath it) and abuts, across a HEAD joint
 * at each end, a long side-wall stub that runs PERPENDICULAR to it (inboard in -Y). Each side-wall
 * stub rests on a grounded foundation by a bed joint — that bed joint is the SPRINGING SEAT the
 * arch delivers its thrust into and the joint this test reads.
 *
 ...
 *
 * So the buggy axis normalises to (-0.903, -0.430, 0) — 43% out of plane — while the end-centre
 * difference the fix uses is (-1, 0, 0), purely in plane. The abutments still split into two ends
 * (their vectors' dot product is -441 + 100 = -341 < 0), so the arch genuinely fires.
 *
 * =====================================================================================
 ...
 *     its length. This proves the fixture routes an arch through corner abutments — the precondition
 *     the whole defect needs — and is a statement about the geometry, not about production.
 *
 *   - THE ARCH FIRED. Each springing seat carries a NON-ZERO in-plane (X) horizontal thrust, equal
 *     and opposite between the two ends. This is the guard that stops the out-of-plane row being
 *     vacuously true: an arch that never thrust would read zero on every axis.
 *
 *   - THE RED CLAIM: each springing seat carries ZERO out-of-plane (Y) force. A flat arch's thrust
 *     lies wholly in the wall plane, so the perpendicular component must be zero at EACH springing.
 *     It is asserted PER SPRINGING and never as a net, because the two ends are equal and opposite
 *     and their spurious Y CANCELS globally even today — the net is zero in both the broken and the
 *     fixed model, so only the per-joint reading discriminates them. Today each springing reads
 *     about +/-15,000 uu of Y; the correct answer is zero.
 *
 *   - IT STILL STANDS. The span reads Supported and nothing is stranded — the fix removes only the
 *     out-of-plane part, it does not change whether the arch holds.
 *
 ...
 * Nothing here measures how far anything moved.
 *
 * WHY Y IS PURELY THE THRUST. Every applied load in this fixture is vertical (gravity, -Z), and the
 * only non-vertical joint is the X-normal head joint, whose shear plane is Y-Z; the span's weight
 * resolves as pure -Z shear on it, so the abutment receives no Y from routing and the springing bed
 * joint's Y component is exactly and only whatever the arch thrust put there. Absent the arch it is
 * zero; the buggy diagonal axis is the sole source of a non-zero reading.
 *
 * NEEDS A TICKING WORLD: NO. FStructure is plain arithmetic over a graph; MakeInterface is plain
 * arithmetic over boxes. Gravity is the piece weights the solver applies itself.
 *
...
bool FCornerHungArchThrustTest::RunTest(const FString& Parameters)
{
	...
	/*
	 * THE FIXTURE'S NUMBERS ARE RATIOS OF PUBLISHED VALUES, so they mean what they say only while
	 * the profile still carries them. Asserted rather than imported: a test that read the profile
	 * would agree with a wrong profile.
	 */
	TestEqual(
	...
	/*
	 * =====================================================================================
	 * THE FIXTURE IS GENUINELY CORNER-HUNG — TowardAbutmentCm[0] IS OUT OF PLANE.
	 * =====================================================================================
	 *
	 * Rebuilt here from the piece positions the solver reads: the vector from the spanned group
	 * (the single span brick) to the first abutment (the left side wall). The reseat records exactly
	 * this as `Arch.TowardEndZero` and hands it to the thrust pass as the push axis.
	 */
	 ...
	 *
	 * GetConnectionForce is the force acting on the joint's PieceB; the in-plane thrust is its X and
	 * the out-of-plane push is its Y. The vertical Z is gravity and this test says nothing about it.
	 */
	...
	/*
	 * THE ARCH FIRED — the guard that stops the out-of-plane row being vacuous. Each springing must
	 * carry a real in-plane thrust, and the two ends push apart (equal and opposite in X).
	 */
	TestTrue(
		*FString::Printf(
	 ...
	 * out-of-plane (Y) force. Today the diagonal TowardEndZero axis pushes about +/-15,000 uu of it
	 * into each springing; the correct answer is zero. Asserted PER SPRINGING, never as a net: the
	 * two ends are equal and opposite so their spurious Y cancels globally in both the broken and
	 * the fixed model, and only the per-joint reading tells them apart.
	 * =====================================================================================
	 */
	TestTrue(
		*FString::Printf(
