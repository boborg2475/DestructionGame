...
| Free-end ladder (**20/30/40/50/60** courses — the heights were mislabelled 10–50 here until 2026-08-12) | 0.3283 / 0.5011 / **0.6740** / 0.8469 / 1.0198 — the test's own extrapolation crosses 1.0 at **58.85** courses (composite-relieved corbel-root reading; NOT the same joint as the sweep's free-end worst, which is unrelieved-half-seat-shaped and crosses between 10 and 20 courses — `FreeEndHeightLadder` pins that one) | `StructureFreeEndHeightTest` |
| Free-end 7×10 and 7×20, oracle λ* | **256.820018**, identical to one ulp across the two heights (the LP's governing mechanism is not the half seat) | `OracleSlowSweep.RigidBlock.FreeEndHeightLadder` |
| Case 7 / case 9 jamb readings (the span pair, relocated onto readings) | **0.26944083288948872** / **0.98502040901419818** at their named joints — 3.6558× apart on a 2.69× span ratio; case 9's is the catalogue's knife-edge (0.985 < 1.0 is an asserted flap watch) | `Acceptance.Wall.SpanIsReadInTheJointNotInTheOutcome` |
| wall-15/16 (header loaded/bare), oracle λ* | **868.623736**, identical to one ulp — the LP needs the surcharge as a *live* load to separate the pair; production separates them 31.56× (0.058203838191552663 / 0.0018443665221594297, this table's own anchors) | same file, `WallsAndLadders` |
