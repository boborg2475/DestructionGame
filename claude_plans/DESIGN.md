...
3. **Build a 2D rigid-block LP as a test oracle only** — **built 2026-08-09/11** (`Tests/RigidBlockOracle.*` core + `RigidBlockOracleSweepTest` sweep). What the measurements said: every §8 corbel ruling upheld by the limit theorem at characteristic bond (the counterweight priced at 22.6× where production reads 0.4%); composite depth cross-validated to 0.03% on the 13/14 pair; the beam rows stand at λ* 1.76/19.2/17.4 with 9.8× material discrimination; wall-08's catalogue verdict is now the outlier of three methods (λ* 324.7 — user ruling pending); the leaning stack's 130× true-margin swing confirmed behind production's height-flat 0.1388. Still owed: rows 9/10/19/20, the 11/12 pier pair, the λ = 3.464 ladder and the stack-bond composite fixture — all beyond the dense solver's measured answering envelope; **the sparse rewrite is now the concrete blocker**, with a live canary (`OracleRefusesAtThisScale`) pinning the envelope.
