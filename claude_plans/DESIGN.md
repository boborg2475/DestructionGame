...
**The composite depth is now BOUNDED, twice over** *(hole found and closed 2026-08-07, [COMPOSITE_DEPTH_DESIGN.md](COMPOSITE_DEPTH_DESIGN.md))*. It was unbounded, and the reason nothing caught it is worth keeping: every corbel fixture happened to have its wall stop where its corbel stopped, so no test could tell a bounded rule from an unbounded one. In a wall *taller* than the cut — which is the wall the game actually renders — the reading goes as `k³/m²` rather than `k³/k²`, so ARCHING_DESIGN's "it crosses over near 36 courses" did not hold there, and the same cut read **safer** under 40 courses than under 13.

The rule is `D = min(masonry above, max(the corbelling body's own depth, λ·|M|/|F|))` with `λ = 3.464`, **and it adds no constant** — both terms are measured lengths. The arm term bounds how far into the wall *above* the cut a corbel may reach, since that masonry has to be dragged in by shear; the floor is the corbelling body itself, which generates the moment, is bonded into one cantilevering mass and needs no shear transfer to be engaged. **The floor can never credit a course of the wall above the cut, because the floor is the cut.**

Two lemmas asserted along the way turned out false and a test caught each — the half-seat lemma (an increment arriving *centred* grows the force without the moment) and the matched-corbel lemma (which silently assumed a half-cell step; the arm actually caps a matched corbel below a 6.4954 cm step, **exactly 2.00× the published per-course limit**). Both are recorded in the design with their arithmetic.

**Still open**: whether the section may credit courses that do not cross the plane it resists across — a stepped corbel of single bricks at half-cell steps has only ~1.9 courses intersecting any vertical plane, yet is credited its whole height. Under investigation; it may subsume the missing stability check above rather than needing one.
...
- **Code:** the scaffold knows nothing special about bricks — it just spawns a **scenario**. Pressing Play can load any scenario, and adding demos never touches the core scaffold.

  **A SCENARIO IS A ROW OF DATA, NOT A SUBCLASS** *(decided in code 2026-08-07, recorded here 2026-08-08)*. This paragraph previously read "a **scenario base class** exists; a brick wall is one scenario that inherits from it", and the implementation deliberately did not do that. `World/DestructionScenarios` is a **catalogue** of `FScenario` rows — a name, the map that selects it, a title, an expectation, how long it holds, the brick centres it cuts, and **the producer that lays it**, carried on the row as a `TFunction` rather than named by an enum a `switch` reads. The reasoning is the one that already governs materials and connection types: a base class or a switch grows a branch per family, and every family added afterwards pays for every family already there, so `Build` never learns what a corbel is. Adding a level is a catalogue row plus a **duplicate** of the map — never a class. It must be a duplicate and never a file copy: a map's `PrimaryAssetId` is `Map:<the inner UWorld object's name>`, so a copy keeps the original's world name, every copy claims the same id, and the editor refuses to open any of them. `Scripts/New-ScenarioMap.ps1` does it correctly and `Content.ScenarioMapsAreDistinctAssets` catches it when it is done wrongly. Twenty-nine rows over three producers today (`Layout::RunningBond`, `Corbel::Build`, `WallCases::Build`), which are the only code any of it needed. **If a scenario ever appears to need a class of its own, that is the drift to stop**, exactly as it is for a material. The argument is restated at the decision itself, in `World/DestructionScenarios.h` on `FScenario::LayStructure`.
