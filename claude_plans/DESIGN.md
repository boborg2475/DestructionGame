...
**Interaction fine print.** The thrust fires only where a group was *spanned* (a hole wider than one brick); a one-cell hole is the moment cap's case, no load crosses it, and its springing carries exactly zero shear. **That means the one-cell cap's equilibrating thrust is never applied or capacity-checked — a fail-open gate and a known gap (§7, item 4).** Moment is not conserved at an arched joint (the missing couple belongs to a neighbour the thrust loads only in the spanned case) — recorded honesty, same shape as the N ≥ 2 rule.
...
| Beam pair: midspan | derived M = 139,288,968 uu·cm → 83.57 MPa = **3.48×** C24's f_m,k; the solver reads **|M| = 0** (deliberate red). Light-wood and heavy-steel rows read ~0.31 / ~0.34 and stand | `BeamAcceptanceTest` |
...
1. **No global equilibrium — statically impossible states read as safe.** A rigid body past its tipping point is *inexpressible* because the model only ever checks joints (§5.7). Behind the corbel ruling's cost, case 12's pier, and partially 10/19/20.
...
4. **The one-cell arching moment cap is granted fail-open** — the 114× relief fires on topology, but the equilibrating thrust is only applied for spanned *groups*, so in the one-cell case it is never applied or capacity-checked. Small fix; queued.
...
6. **Arch thrust neither walks down the pier nor overturns it** — a flat span on one-brick piers should shove them over (BIA TN 31). Acceptance case 12; same missing check as item 1.
...
2. **Interim overturning guard**: free-body check of the maximal bonded body about its bearing edge. Explicitly disposable — the leaning-stack acceptance case is its red test.
3. **Build a 2D rigid-block LP as a test oracle only.** Run every fixture and anchor through it; diff verdicts. Converts "is composite action right?", "is λ right?", and the red acceptance rows from rulings into measurements at zero risk to the shipping solver. *The pivotal investment.*
...
- **2026-08-08 — case 11 ruled twice; the second ruling is the keeper: it STANDS.** First ruled local-loss on BS 5977's 300-mm-above-apex gate; then the user directed that **published design rules are not collapse predictors and must not be treated as gospel — evaluate the physics honestly.** Worked honestly: the ~120 kg panel rides a 60-cm-deep bonded deep beam (span/depth ≈ 2.3) at ~0.03–0.04 MPa of bending — 5–10% of mean bond strength — and the 66-cm piers take the thrust easily. The model agrees (0.362193, at the reveal corner). That direction is a **standing instruction** for every future verdict. The genuine "no room to arch" discriminator remains case 8 (one course of cover), which stays red.
- **2026-08-08 — strength basis: mean values, for realism** *(user)*. See §3; re-anchor after the LP oracle, in one verified pass.
- **2026-08-08 — case 12's rewrite approved** (6-cell span on 1-cell piers with a survivor region, isolating pier width; the encoded case varies span *and* pier at once and near-duplicates case 9). Queued.
