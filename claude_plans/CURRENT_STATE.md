...
Last updated: 2026-07-31 (seeded structural fuzz added to the suite)
...
- **Four core systems are in**, all test-driven. 20 tests, all green.
...
**Anonymous namespaces in test files are a unity-build hazard, currently defused by luck.** `StructureTest.cpp`, `ConnectionTest.cpp` and `ConnectionStrengthTest.cpp` each declare `Mortar` and `MakeNaN` in an *anonymous* namespace, which is the same namespace once two of them land in one unity blob — a redefinition error. UBT's adaptive unity has kept the changed file out of the blob every time so far. `StructureFuzzTest.cpp` uses a named namespace to sidestep it; the older three would need the same treatment if a full unity build ever merges them.
