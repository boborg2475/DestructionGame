...
1. **Sparse LP solver rewrite** — promoted from a cost note to the concrete blocker by the sweep's measured envelope: the dense simplex refuses (fail-closed post-solve verification) on every fixture ≥ 119 blocks and on the 74-block free-end 7×10; still owed behind it are red rows 9/10/19/20, the case-11/12 pier-width pair, the λ = 3.464 free-end ladder, and the stack-bond composite fixture. The live canary is `OracleRefusesAtThisScale` in `RigidBlockOracleSweepTest.cpp` — a working sparse solver fails it loudly; promote, don't delete. The slow sweep's opt-in convention: `Automation RunTests OracleSlowSweep` (~4.5 min; its name deliberately sits outside the `DestructionGame` filter prefix).
