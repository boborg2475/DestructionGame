...
>
> **Status, stated plainly: a closed sandwich is a HEURISTIC with §5.6's fail-closed fallback armed, not a certificate.** The optimistic side is proven; the pessimistic side has had one demonstrated counterexample removed and has never been shown to be a bound. **The gating follow-up** (specified, and the next work item): a wall the LP prices **below 1.0** whose ground-anchored strip still stands — the only fixture shape that can exercise the pessimistic side on a proper subset. The leaning stack is currently the catalogue's only sub-1.0 structure, which is exactly why the hole cannot be probed today.
>
...
| R2 | ~~The regional sandwich never certifies.~~ **FIRED on the original form** (95–100% regions, 1.68×, a false certificate wrong by ~40×). **The repair was measured 2026-08-15 and SPLIT the risk in two.** *Size* is no longer a risk: ground-anchored strips certify at **27.5% for 0.322×**, 10 of 10 deletions, height-invariant. *Soundness* is now the whole of R2: the pessimistic side is a heuristic, not a bound, and the "0 false of 20" statistic **cannot fire in the direction that matters** (§5.3 box). | Lever 2 is worthless and R1 fires. | **Next, and it gates treating a sandwich as a certificate:** build a wall the LP prices **below 1.0** whose ground-anchored strip still stands — the only fixture shape that issues a certificate about a proper subset of an infeasible structure. Until then run sandwiches with §5.6's fail-closed fallback armed. Also owed: close the three named holes (interface joints unchecked, carried component's internal joints dropped, arbitrary area-proportional split). |
