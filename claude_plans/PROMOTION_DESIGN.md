...
- **The fallback.** An LP that refuses — and the oracle refuses today on case 22's 218-block family, MEASURED — must fail closed to something that keeps the game playable. That something is the answer production already gives.
...
- **Standing is cheap and falling is expensive**, and that asymmetry is the right way round for a game. Proving feasibility can stop the moment the phase-1 infeasibility sum reaches zero — no optimality proof needed. Proving *in*feasibility requires running phase 1 to optimality. Most solves in play are "still standing"; the expensive ones happen during a collapse, where a frame of hitch is hidden by the collapse itself.
...
### 5.2 Lever 1 — stop asking for λ\* (the largest, and unmeasured)

Discussed in §3.2. The whole of wall-01's 58,605 pivots is phase 2 climbing λ from 0 to 272.2; a feasibility problem at λ = 1 does not take that walk. It also gets to stop early on the standing case — the moment the phase-1 infeasibility sum reaches tolerance the answer is *proved* and no optimality proof is needed.

**ESTIMATE: 10–100× on standing structures, less on falling ones** (an infeasibility proof requires phase 1 to optimality). The basis for the estimate is that phase 2's path is the entire measured cost and phase 1 on a warm-started, nearly-feasible problem is short. **This is not on the recorded roadmap and it is the cheapest thing to measure**: it is a test-side change — pose the existing fixtures with `bGravityIsLive = false` and compare pivot counts and wall time against the λ\* rows already pinned.

### 5.3 Lever 2 — regional decomposition, with a certificate rather than a hope

The roadmap states the intuition: "a deleted brick provably cannot affect masonry beyond a bonded distance". As stated it is **not true in this model and the tree contains the counter-example**: case 21's mechanism runs the full length of the jamb chain to the ground, and the abutment ladder measured λ\* responding to jamb width. A thrust path is not local.

What *is* sound is a two-sided sandwich, and it is provable rather than hopeful:

- Solve the region with its boundary blocks pinned **grounded**. That grants the region an artificial foundation, so it is **optimistic**: if this problem is *infeasible*, the full structure is infeasible too, and the mechanism is genuine.
- Solve the same region with its boundary **free** (the boundary blocks carry their own weight and nothing holds them). That is **pessimistic**: if this problem is *feasible*, the full structure is feasible, because the found force system extends.
- If the two agree, the answer is certified and the rest of the wall was never needed. If they disagree, grow the region and repeat.

Region growth is bounded by the structure, and in the common case — a brick pulled from the middle of an intact wall — the two should agree at a small radius. In the uncommon case (a cut that severs the last thrust path to the ground) the region grows until it reaches the ground, which is the correct behaviour and is exactly when the player deserves to see a whole wall move.

MEASURED anchor for what a region costs today: the 84-block / 207-joint fixture solves λ\* in **0.61 s**. A region of that size needs to answer in ~50 ms to fit two solves inside a 100 ms budget: **12× needed**, against a lever (§5.2) estimated at 10–100×, plus warm starts. That is the first arithmetic in this document where the target and the levers are the same order of magnitude, which is why this is the recommended path and why §5.2 must be measured before anything is built.

**Everything in this section is unproven.** No regional solve has ever been run in this tree; the sandwich has never been tested; the convergence radius is unknown; and it is possible that the free-boundary side is so pessimistic that it never certifies anything, in which case the sandwich degenerates to "always grow to the whole structure" and the lever is worthless. §6 puts the experiment that would reveal that first.
...
- **Case 22's 218–371-block refusal — IN SCOPE, Slice 0a, blocking.** Reasons in §6. The refusal is on a catalogue wall at a size that sits inside any region, and the two live hypotheses (the spurious-unbounded arm, NB-7's absolute-tolerance scan on the artificial pivot-out) are both cheap to instrument. Also within it: the ~5-line `WhyNot` split, which needs its own red first because a diagnostic string no test reads is production code without a failing test.
...
|---|---|---|---|
| R1 | **Latency does not close.** The feasibility reformulation and the regional sandwich under-deliver; a region solve stays ~0.6 s. | Synchronous LP authority is impossible at scenario scale; the design becomes asynchronous or hybrid, which changes everything downstream. | **Slice 0b**, before any production code: re-pose six pinned fixtures at λ = 1 and time them; run the sandwich on one wall at radius 2/4/8. Kill criterion: 84 blocks not under ~50 ms with warm starts. |
| R2 | **The regional sandwich never certifies.** The free-boundary side is so pessimistic that regions always grow to the whole structure. | Lever 2 is worthless and R1 fires. | Same slice: measure how often the two sides agree at radius 4 over ten deletions in an intact wall. |
...
| R4 | **The LP refuses in production.** Case 22's family refuses today; the spurious-unbounded arm is unreached rather than sound. | A refusal mid-cascade with no fallback is a frozen or wrongly-collapsing wall. | **Slice 0a** plus a run of the whole catalogue at production scale counting refusals by reason, once `WhyNot` distinguishes them. |
...
**D2 — Latency budget and approach.**
...
*Evidence:* the project's own history. The one-cell thrust gate's beam fallout was invisible for two days and is the reason the `DropsToday` pins exist; the mean re-anchor's predictions were wrong on five separate counts and only measurement caught them; the case-22 refusal is a live unexplained solver finding *right now*. The aggressive alternative — promote and delete in one slice — would land a change touching every verdict in the catalogue behind a single green run, which is exactly the shape this codebase has been burned by.
