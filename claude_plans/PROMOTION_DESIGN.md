...
Predicted effects — **MEASURED 2026-08-21 (Slice 0d, pins in `RigidBlockOracleSweepTest.cpp`; the predictions were written first, disagreements recorded):**

- **Case 21: does not flip — CONFIRMED, and it moved less than predicted.** Predicted λ\* somewhere in 6–14; measured **15.117, a 12.3% drop, still standing**. So bond bending is an even more minor term here than the estimate — the residual is the non-brittle friction+compression mechanism Slice 0c attributed, which first crack does not touch. The brittleness rule is right and does not close the disagreement, exactly as argued.
- **Beam row 1 (glue line, C24): FLIPS to Falls — CONFIRMED, λ\* = 0.88258, near the ~0.9 estimate.** The correct verdict (member fails in bending) by a slightly wrong route (the glue line cracking). **This is a catalogue verdict change and therefore the user's ruling** (§8), due when step 4 turns the flag on in production — Slice 0d only measured it behind the default-off flag and asserts the λ\* drop, never the flip.
- **Beams land at exactly control/3** where N≈0 (pure bending); movers with net tension across a multi-joint run cut deeper than the per-joint 1/3 floor by redistribution (corbel A 0.2786). A single tension-bending joint's ratio is (1+k)/(1+3k) ∈ (1/3, 1] and cannot go below 1/3 — the sub-1/3 is a multi-joint effect, not per-joint net tension. Dry-stone rows return **bit-identical** (the keyed-on-f_t>0 crux; the discriminating bite-prover is still owed — see the §6 slice note).
...
**Attribute the 4.768 — DONE 2026-08-21, `OracleSweepFast.RigidBlock.Case21ResidualAttribution` (the pin for every number in this paragraph).** The 2026-08-13 ruling's stated reason — the LP mobilises brittle cohesion along a whole chain — was known to account for 72% of the stand and not for the rest, and the residual 4.768 was unattributed. It is now measured, and it **confirms §4.2 rather than surprising it**: the residual is a *non-brittle, no-tension, friction-mobilised compression* mechanism. The probes, on the same 83-block fixture (seven solves, ~8 s):

- **Friction is the load-bearing residual, and crushing is inert.** Zeroing the jamb crushing cap leaves λ\* bit-identical to the 4.768 control (rel diff 1.86e-15) — crushing sits ~500× clear of demand and does not bind. So what holds the cohesion-stripped wall up is compression plus Coulomb friction-shear, with no brittle bond involved — exactly the "27.7% is friction and compression, not brittle" of §4.2, now measured rather than asserted.
- **The counter-intuitive row, and it is not a cap.** Zeroing the friction coefficient *raises* λ\* to 9.935. That is **not** friction capping a compression arch: with cohesion already gone, μ = 0 forbids all bed-joint shear and in the same move lifts the n⁺ ≥ n⁻ coupling that had held the joints in compression, so the 0.70 MPa flexural tensile bond (which the probe leaves intact — it zeroes Coulomb cohesion, not the tensile bond) becomes usable and the wall re-forms on a *distinct, higher, bond-tension* mechanism. Provable: any μ = 0 optimum in net compression everywhere would be feasible for the μ = 0.75 control and so bounded by 4.768; 9.935 > 4.768 therefore means net tension is mobilised. **Measured, not just argued:** a fifth probe zeroing the tensile bond on top of μ = 0 collapses that mechanism to 3.747 — below even the frictional residual — proving the 9.935 was carried by the brittle bond, not by the residual.
- **The chain-versus-bearing split inverts the "thrust runs to ground" intuition for the cohesion term.** Removing cohesion in the *bearing courses* alone reproduces essentially the whole 17.24 → 4.768 collapse (4.76816), while removing it only in the *run to ground* reaches just 5.962 — so cohesion's contribution is local to the bearing courses at the jamb top, not distributed down the chain. (Friction, separately, *does* run the chain — the two attributions are kept distinct.)

**Consequence for the promotion:** §4.4's governance rule stands unchanged and is *strengthened* — the residual STANDS on a non-brittle active set (friction + compression), which is exactly the case the rule authorises, so case 21's inverted red rests on the catalogue ruling and the bond-mobilised flag, not on an unexplained number. R5's contingency (residual "turns out to be something that ought not to be there") did not fire.
...
>
> **The repair — MEASURED 2026-08-15 (`RepairedRegionalSandwich`), and the two halves came out very differently.**
>
> **Repair (2), ground-anchored growth: measured, and it works.** Full-height strips to the foundation instead of balls around the deletion. A 12×12 wall certifies from **41 of 149 blocks — 27.5% — at 0.322× a global solve**, with **10 of 10** deletions certifying, against the refuted form's 142 of 149 and 1.68×. The fraction is height-invariant (27.7% at 18 courses) though the *count* is not (41 → 62). Forced rather than chosen: the free side is provably infeasible for any region containing no grounded block.
>
> **Repair (1), the dead surcharge: essentially untested on the arm that matters, and the headline statistic cannot detect its failure.** Three findings, the second from review:
> - The **carried set is empty at every wall strip width**, so on the wall arm the repaired pessimistic side is *bit-identical to the refuted one* — every wall certificate rests on repair (2) alone, and the surcharge is exercised only by the one collapse fixture.
> - **"0 false certificates of 20" cannot fire in the pessimistic direction.** 19 of the 20 concern structures the LP already prices feasible and the 20th is the whole structure, so no certificate is ever issued about a *proper subset of an infeasible structure* — the counter exercises the optimistic side (already sound) on every row. This is TRAPS' "an invariant asserted over fixtures that all share a hidden property is not an invariant", and it means the statistic is **no evidence, not weak evidence**.
> - **At least three unsoundnesses remain, not the one the slice first named.** (i) The interface joints are dropped — the surcharge is applied directly to the region block, so the joint that must actually transmit it is never checked; review worked the stack's joint 5-6 needing T = 636,000 uu against a 412,562 uu cap. (ii) The carried component's own internal joints vanish, so a hanging component failing internally leaves the region certifying. (iii) The area-proportional split across multiple interfaces is an arbitrary fixed choice where reality lets the LP pick.
>
> **What repair (1) DID buy, and it is a real finding**: the surcharge must act at the omitted material's **centre of gravity, not at its contact with the region**. Mutation X8 moves it to the contact and *re-creates the false certificate* — verified independently in review by hand statics, the two cases 2.36× apart. The load-bearing content of the repair is *where* the weight acts.
>
> **Status, stated plainly: a closed sandwich is a HEURISTIC with §5.6's fail-closed fallback armed, not a certificate.** The optimistic side is proven; the pessimistic side has had one demonstrated counterexample removed and has never been shown to be a bound.
>
> **THE GATING FOLLOW-UP RAN, 2026-08-16, AND THE ANSWER IS THAT IT IS NOT A BOUND — `OracleSweepFast.RigidBlock.SubUnityWallCertificate` pins it.** The fixture the hole needed is a wall carrying a **leaning chimney**: the 12×12 wall this box already measures, with the 30-course leaning stack's own chain mortared onto the rightmost full brick of its top course and leaning out past the wall's end. The structure has no admissible equilibrium (its λ\* is the bare stack's, pinned in the test), the same wall without the chimney stands, and a ground-anchored full-height strip through a mid-wall deletion **certifies "the whole structure is feasible" at four of six strip widths** — the smallest about 23% of the structure. The certified strip's own λ\* against the structure's makes it wrong by **three orders of magnitude**, where the refuted form's was 41.96×. The mechanism is pinned as an identity rather than argued: the certified region problem is **bit-identical with the chimney attached and without it** — same blocks, same joints, same two pivot counts — so its answer cannot depend on the material it dropped, and a region whose answer cannot depend on the omitted material cannot bound a structure that includes it.
>
> **And it exercises TWO of the three named unsoundnesses, not the one it was built for.** At the narrow widths the carried-set rule is BLIND (hole (1): the chimney reaches the ground down the wall's own columns, so it is charged nothing). At the width where the chimney's root is pulled in as a shell block the rule is NOT blind — it charges all thirty chimney blocks at their own centre of gravity — **and the strip certifies anyway**, because the surcharge is delivered through the root's bed joint, which stands, while the chain's twenty-nine internal joints, the ones that fail, are in no problem at all. That is hole (3) measured: **a surcharge carries a force and a moment and NO STRENGTH.** The region sees the failure only at the width that contains the failing *joint*.
>
> **What this settles and what it does not.** Settled: §5.6's fail-closed fallback is mandatory on every region solve, and no sandwich may be treated as a certificate. Not settled: the repair. The hole-(3) rung says the obvious next repair is larger than it looks — the region would have to carry the charged component's internal joints and the interface joint as well as its weight. **Honest limitation of the fixture**, stated in the test header too: it is infeasible *before* the deletion, so the counterexample is not deletion-caused. The claim it refutes is unconditional, so that does not weaken it, but a deletion-caused non-local failure (the wide-opening cover with jambs standing) would be strictly stronger and is logged in CURRENT_STATE.
>
> **Cost honesty**: 0.322× is the *wall* arm. The collapse arm costs **2.0×** at closure (326 pivots against a 163-pivot global solve) and 8× across its full growth ladder — worse than the 1.68× the refuted form was condemned for. Both numbers are real; quoting only the first oversells the lever.
>
> **Scope condition**: full-height strips only. The carried-set rule under-charges material above a *partial-height* strip in a laterally-connected wall, so partial-height regions are outside what this measurement certifies.
...
### 5.4 Lever 3 — warm starts — **MEASURED 2026-08-18 AND REFUTED**

> **The primal warm start is not a latency lever. It is negative.** Measured in `OracleWarmStartTest.cpp` (predictions written first, all five missed low):
>
> | fixture | blocks | cold pivots (ph1) | warm pivots (ph1) | ratio | predicted |
> |---|---:|---:|---:|---:|---|
> | gate 8×10, one brick | 84 | 343 (342) | 342 (342) | **1.003×** | 1.6–5× |
> | gate 8×10, whole top course | 84 | 291 (290) | 1543 (1543) | **0.189×** | 1.2–2.5× |
> | gate 8×10, ground cut (falls) | 84 | 90 (90) | 77 (77) | 1.169× | 0.7–1.5× |
> | 12×12, one brick | 150 | 919 (918) | 4192 (4192) | **0.219×** | 2.5–6× |
> | wall-01, one brick | 375 | 4764 (4763) | **abandoned** | — | 5–18× |
>
> **It gets worse with size** — 1.003× at 84 blocks against 0.219× at 150, which is the comparison that decides between "the repair is local" and "thrust is not local", and it decides against locality. **It is slower in wall-clock on every row** (extra factorisation, denser basis). **The entire phase-2 saving is one pivot, forever** — a warm basis arrives with λ at its cap, so warm phase 2 is 0 pivots against cold's 1, and every ratio above is phase 1. And **at wall scale it is unusable**: the mapped 12,459-column basis goes singular at pivot 64, the first periodic refactorisation, and the solve is abandoned for a cold retry. With the sign-flip repair gated off it fails identically at 64, so the fragility is the *mapped basis*, not the repair.
>
> **The sharpest single datum**: the gate row carried **2,600 of 2,628 columns — 99% of the previous optimum — and phase 1 still cost 342 pivots against a cold 342.** Redistributing what six deleted joints were carrying costs as much simplex work as building the entire force system from nothing.
>
> **What this does NOT refute is the dual simplex** — the old basis is dual-feasible and primal-infeasible, which is textbook for it, and it would never walk phase 1's road at all. **But carry the warning**: a dual simplex starts from the *same mapped basis* that just went singular at wall scale, so it inherits that conditioning wall. On this evidence, budget no margin on warm starts of either kind at 375 blocks. §12 D2″'s "warm starts are the margin" is **spent**.

After a deletion the previous optimal basis remains dual-feasible for the reduced problem, so a dual-simplex re-solve is the standard incremental playbook and the roadmap names it. *(Written before the measurement above. The premise is still true; what changed is that the primal route to exploiting it has been priced and is worthless, and the dual route inherits a known conditioning failure.)* Two honest notes: **the oracle has no dual simplex at all**, so this is new machinery of roughly the size of the existing phase-2 loop (reusing `FBasisFactor`, `FStandardForm` and the eta file); and the roadmap's own "~10× pivot collapse expected" for warm starts across sweep rows is an expectation, not a measurement.
...
**Slice 0c — attribute case 21's residual capacity. DONE 2026-08-21 (`OracleSweepFast.RigidBlock.Case21ResidualAttribution`).** Landed as seven probe rows, not the planned four: the friction/crushing/both probes plus the chain-versus-bearing split, plus a fifth probe (μ = 0 *and* tensile bond zeroed) added on review to turn a feasibility proof into a measurement. **Proved: the 4.768 is a non-brittle friction+compression mechanism (crushing inert); the 9.935 that appears when friction is removed is a distinct bond-tension mechanism, not a cap being lifted; cohesion's contribution is local to the bearing courses, not run to ground.** The full attribution and its numbers are in §4.4 (the pin's owner). Result confirms §4.2 and strengthens §4.4's governance rule; R5's contingency did not fire.

**Slice 0d — the first-crack rows, behind a problem flag, measured across the whole sweep. DONE 2026-08-21.** Rows wired behind `FOracleProblem::bFirstCrackRows`, gated on the flag and per-joint on f_t > 0; written predictions first, then the run, then the disagreements recorded (§4.3 above). Review independently re-derived the two LP rows and confirmed them correct. **Proved: bonded-bending capacity cuts to control/3 (deeper across multi-joint runs), case 21 stands (12.3% drop), beam row 1 flips to Falls, and dry rows come back bit-identical.** Two items still owed, both logged in CURRENT_STATE: the keyed-on-data bite-prover (M2 does not fire on standing dry fixtures because their first-crack row is the slack kern condition — needs an eccentric-optimum dry witness), and the exhaustive slow-wall flag-on re-pin (deferred as disproportionate). The beam-row-1 flip is the user's ruling, due at flag promotion, not at 0d.
...
**Slice 5 — delete the heuristics. SUPERSEDED BY D8 (finding 2026-08-27; scoped read-only).** `ReseatSpannedGroups`, the composite-depth walk, the one-cell thrust gate and the arching moment cap. This slice assumed the LP would be the **full** cascade authority, so the router and its heuristics would be dead everywhere. **D8 scoped LP authority to ≤ 200 blocks and kept the ~1,220-block flagship and the giant corbels on the router indefinitely — so the heuristics are NOT dead code; they are the router's break authority ABOVE the cap.** And every acceptance fixture is below the cap, so the suite cannot see what a deletion does to a large structure. The regime map (`SolveLoads` runs the heuristics at every scale, writing `ConnectionForces`/`ConnectionMoments`/`ConnectionCompositeDepthCm`; `BreakByEquilibrium` owns the verdict ≤ cap and reads NONE of them; `BreakByCapacitySweep` owns it > cap and reads ALL of them; the readout accessors read them in every regime):
- **≤ cap, verdict:** the LP owns it, the heuristics are inert for breaks — but they still feed the READOUT, and re-basing the readout is Slice **6**'s job (D5 keeps the router as the strain readout *through* Slice 5).
- **> cap, verdict:** the heuristics ARE the physics. Deleting composite depth can flip an above-cap bonded corbel stand→fall (`AHundredStepCorbelMustComeDown` is the one above-cap Class-A verdict that reads it — the giant ~6,800-piece corbel); deleting the arching cap before its `HasArchingAbutment` gate re-opens the pre-2026-08-09 fail-open defect — all invisible to the below-cap suite.
- Even the spanning/thrust deletion (the only one no *fixture* exercises above the cap) is unsafe for the game's own vision ("players build structures"): a player-built > 200-block wall WITH an opening would lose the router's arching and collapse wrong, with no test to catch it. It only *looks* safe because today's playable set is fixed scenarios (a solid flagship, no openings).
**Ruling: the router heuristics are RETAINED as the above-cap break authority; there is no clean deletion Slice 5 can make independently.** The "delete the heuristics" intent revives only if the LP ever takes the flagship (needs a latency lever that does not exist — D2⁗); their below-cap *readout* role is removed by Slice 6 when it re-bases the readout off the router (§3.5 min-violation LP). Slice 5 is closed as a finding, not a deletion. (The full regime analysis is in the 2026-08-27 scoping record.)
...
|---|---|---|---|
| R1 | ~~Latency does not close.~~ **DID NOT FIRE at region scale, 2026-08-15 — but FIRED at wall scale, 2026-08-18, and the response is D2⁗'s size-scoped authority rather than another lever.** A region solve is **0.0666 s**, not the feared ~0.6 s; the deletion-caused re-solve production actually runs is ~51 ms, essentially at budget. Wall-01 stays at 26 s, warm starts are refuted (§5.4), and the per-iteration set is worth ~1 order against ~260×. *Superseded detail:* 1.33× short with warm starts unbuilt. *Residual risk, unchanged:* wall-01 still needs 26 s and scenario scale is ~3.3× that, so synchronous authority depends entirely on regions being small — which R2 now owns. | Synchronous LP authority is impossible at scenario scale; the design becomes asynchronous or hybrid, which changes everything downstream. | Run. The remaining measurement is warm starts (unbuilt, roadmap expects ~10×). |
| R2 | ~~The regional sandwich never certifies.~~ **FIRED on the original form** (95–100% regions, 1.68×, a false certificate wrong by ~40×). **The repair was measured 2026-08-15 and SPLIT the risk in two.** *Size* is no longer a risk: ground-anchored strips certify at **27.5% for 0.322×**, 10 of 10 deletions, height-invariant. *Soundness* **FIRED 2026-08-16**: the gating fixture was built and **the pessimistic side is NOT a bound** — a wall carrying a leaning chimney issues four false certificates, the smallest about 23% of the structure, and the certified region is bit-identical with the failing part attached and without it (`OracleSweepFast.RigidBlock.SubUnityWallCertificate`, §5.3 box). | Lever 2 is worthless and R1 fires. | **Done — and the answer is that a closed sandwich is a heuristic. §5.6's fail-closed fallback is now mandatory on every region solve.** What remains is the repair, and it is larger than it looked: the same fixture shows a strip certifying while it charges the failing material IN FULL, because the surcharge replaces a component's joints with a point load and carries none of their strength. So holes (2) and (3) are one repair — keep the interface joint and the charged component's internal joints in the problem — and hole (1) needs the carried-set rule to stop treating "has a ground path" as "is not carried". Still owed beside them: the arbitrary area-proportional split. |
...
| R5 | **Brittleness cannot be answered at all** — the first-crack rows do not flip case 21 (expected) *and* the residual 4.768 turns out to be something that ought not to be there. | The 2026-08-13 precondition is unsatisfiable as written and the user must rule again before promotion. | ~~**Slice 0c**, four solves, 4.4 s.~~ **RETIRED 2026-08-21: the residual arm did not fire.** Slice 0c measured the 4.768 as a non-brittle friction+compression mechanism (§4.4), exactly what §4.2 predicted — nothing "ought not to be there". Only the first-crack-does-not-flip half remains, and it is expected, not a risk. |
...
**D2⁗ — 2026-08-18, the ruling: LP AUTHORITY IS SCOPED BY STRUCTURE SIZE** *(user, with both remaining measurements in hand; DESIGN §8 carries the ruling)*. The latency plan is closed, because the two levers left were both measured and both refuted. **Warm starts (§5.4): negative** — worse with size, slower in wall-clock everywhere, unusable at wall scale, and 99% of a previous optimum buys nothing. **Regions (§5.3, D2‴): sound in one direction only** — region infeasible ⇒ structure infeasible holds; region feasible ⇒ nothing, unrepairably, because a load-only surcharge carries force and moment but no *strength*. What is left to try is the per-iteration set (Markowitz, presolve, Devex, refactorisation cadence), collectively worth perhaps one order against a **~260×** need at wall-01 — so the honest position is that no lever is coming.
**The decision**: promote the LP to full cascade authority **below a measured block threshold**, and keep the two-tier router above it. Small structures already re-solve in **~51 ms**, inside the 100 ms budget with no lever at all, so the promotion is real where it lands. The threshold is itself a measurement and must be taken rather than guessed — the natural experiment is a size ladder against the budget, and it should be taken with the deletion-caused re-solve (what production runs) rather than the intact solve (what §5.2 measured).
**What this costs and what it does not.** It costs the flagship 1,220-piece scenarios, which keep today's routing physics indefinitely, and it costs the "one mechanism decides everything" framing — the promotion is now explicitly about *part* of the game, and the boundary must be stated in the docs and legible in the game rather than hidden. It does not cost the physics: real limit analysis runs wherever it runs, and the sound direction of regions (proving collapse) remains available at any scale if a future slice wants it. **The recurring finding underneath all three refutations** — case 21's jamb chain to ground, regions certifying only when they reach the foundation, and now 99% of a basis buying nothing — is one sentence: *thrust is not local, so neither is the arithmetic.*

**D2‴ — 2026-08-16, the gating fixture built: THE PESSIMISTIC SIDE IS NOT A BOUND, and the word "certificate" must leave the design's vocabulary.** §5.3's box carries the finding and `SubUnityWallCertificate` pins it. What it changes for D2″ below: consequence (i) stops being conditional — a closed sandwich is a heuristic *full stop*, not "until the fixture exists", and §5.6's fail-closed fallback is armed on every region solve as a standing requirement rather than an interim one. What it does not change: the size answer, which stands. **What it adds to the plan:** the repair implied by hole (3) is a bigger piece of work than the surcharge was, so any slice that proposes to trust a region must price *that* first — and the alternative D2″(iii) already names (probe once, else solve globally) does not need trustworthiness at all, which now looks like its main argument rather than its cost argument.

**D2″ — 2026-08-15, the repaired sandwich measured. Regions CAN be small; the pessimistic side is still not a bound.** The §5.3 box carries the numbers. What this changes for the plan: the size question is **answered yes** (27.5% of a wall, 0.322× a global solve, 10 of 10 deletions, height-invariant fraction), so synchronous authority is no longer gated on decomposition being possible — it is gated on decomposition being *trustworthy*, which is a different and narrower question. **Two consequences to carry forward.** (i) Until the sub-1.0 wall fixture exists, a closed sandwich is a heuristic and §5.6's fail-closed fallback must be armed on every region solve — that is not a new policy, it is the existing one becoming load-bearing. (ii) The scenario-scale arithmetic is now an **estimate stacked on two different fits** and must be treated as such: 311 → 553 pivots across 12 → 18 courses extrapolates to ~1,150 for 30 courses on a *super-linear* fit while the region-size extrapolation beside it is *linear*, and converting pivots to ~0.15 s assumes ~2× the 12-course per-pivot cost — three estimates, one conclusion, against a 0.100 s budget with warm starts (~10× hoped, unbuilt) still the margin. Do not quote 0.15 s as a measurement.
**(iii) The cost distribution is close to the worst possible shape, and this is a design observation rather than a measurement quibble.** The lever is **0.322× when the answer is "it stands"** and **2.0× at closure / 8× over the growth ladder when the answer is "it falls"** — and falling is the case a player's deletion is *interesting* on. Regional decomposition is therefore cheap exactly when nothing needed deciding and expensive exactly when something did. The mechanism is plain: a collapse grows the region until it reaches the whole structure, so you pay for every intermediate solve *and* for the global one you end up doing anyway. **The practical shape that falls out, specified but unmeasured**: treat the narrow strip as a *probe* rather than the start of a ladder — if the sandwich does not close at the first width or two, abandon growth and solve globally once. That caps the cost at roughly one global solve plus a cheap probe, instead of 8×, and it costs nothing on the common standing path. Worth measuring before any region ladder is written into production.
...
**D6 — Slice 2 is additive-only; the dry-stack red re-scopes to Slice 3.** *(Ruled 2026-08-27 by the orchestrator under the "decide, don't ask" delegation, with the read-only Slice-2 design in hand; recorded here for audit and open to veto.)* The Slice-2 design surfaced that the two in-flight reds pull opposite ways: `ABodyOnTwoLoadPathsPastTippingMustFall` needs the gate to **add** a break (production stands it — the guard's N≥2 blind spot; a pure guard-replacement), while `APureCompressionDryStackStandsByGeometry` needs the gate to **suppress** a release (production fells it through the **joint sweep's** no-tension kern break, NOT the guard). Suppressing a release is §3.7's release-rule change, assigned to Slice 3. Doing it in Slice 2 as a whole-structure boolean veto ("LP stands ⇒ release nothing") would introduce a real new error — it over-protects genuine local losses, e.g. **row 20** (LP stands the wall globally while 2 teeth correctly drop). *Decision:* **Slice 2 replaces the guard additively (greens two-load-path only) and the dry-stack red closes at Slice 3**, where the per-piece dual can veto a spurious release without protecting row 20's teeth. This keeps Slice 2 the "smallest authority increment / one topology" the §6 sketch intends.
- **D6-b — pose:** the gate asks **feasibility at λ=1** (`bGravityIsLive=false`), not λ\*. They give the identical Stands/Falls boolean (§5.2 cross-validated it on every fixture), but feasibility is 5–16× cheaper and inherits the *measured* ~84–104-block band (`DeletionResolveLatencyLadder`), where the λ\* pose would need a new, smaller, un-measured cap. No dual/Farkas — that is still Slice 3. *(This overrides the design pass's lean toward λ\*, whose own latency analysis favours feasibility.)*
- **D6-c — a block cap, not a pivot cap** (per the ladder finding: wall-clock grows ×174 vs pivots ×11.9), set at the conservative end of the measured band, with fail-closed to the router (no overturning check) above it. The guard only ever fired on single-bearing bodies well below the cap, so deleting it does not regress the >threshold path — the fallback needs no overturning heuristic.
- **Fallout to re-measure (predictions-first, never scaled), each an orchestrator verdict ruling:** the leaning-stack firing rung (guard artefact — moves under plastic no-tension); corbel A must stay STANDS (Class A anchor); and any fixture the global-infeasibility break arm now fires on that the guard's per-body moment test did not. `OverturningGuardBasisTest` dies with the constant; `LeaningStackAcceptanceTest`'s guard prose/rung is rewritten. Rows 10/19/beams do NOT go green here (that needed the veto) and beam row 1 does NOT flip (first-crack stays off in Slice 2).

**D6 — WHAT LANDED (2026-08-27, `FStructure::BreakByEquilibrium`).** The gate came in cleaner than the fallout plan feared: **the leaning-stack family did NOT move** — the LP fells the stacks identically to the deleted guard (same drop counts, same rung), so the guard was well-calibrated on its home turf and there were NO catalogue-verdict rulings to make. The **coarse attribution** that Slice 2 uses, so a decision made in code lives here: on a Falls verdict the gate floods non-grounded pieces into maximal bonded sets (index-ordered, deterministic), each set's bearings being its intact joints to grounded pieces, and acts ONLY on a **single clean ground-borne body** — declining on 0 or ≥2 such bodies (fail-closed; ≥2 is untested and deferred). Before severing it runs a **remainder-feasibility check**: it re-solves with that body's blocks excluded (weight and restraint), and severs only if the remainder stands — otherwise it declines, because a distant unsupported region the router already owns can make the whole-structure LP read Falls and the naive attribution would then over-release an innocent standing body (a real bug the `Locality` acceptance test caught during the build; the remainder check is the fix). This needs a second feasibility solve on the collapse path — an open latency item near the cap (CURRENT_STATE). The dry stack stays red exactly as D6 intends (additive-only; the joint sweep fells it, the gate does nothing). NaN/refusal are fail-closed to decline, never to a spurious break (verified in review).

**D7 — Slice 3 splits into 3a (oracle mechanism) then 3b (release wiring); determinism is the gate; row 20 is a Slice-4 ruling.** *(Ruled 2026-08-27 by the orchestrator with the read-only Slice-3 design in hand; open to veto.)* Slice 3 makes the LP name WHICH pieces fall via the phase-1 **dual** (the Farkas certificate = the kinematic collapse mechanism), replacing Slice 2's coarse whole-body flood + remainder check. It is split:
- **3a — oracle-side, no production behaviour change:** extract the mechanism (one BTRAN with phase-1 costs at the infeasible arm, `RigidBlockOracle.cpp:~1595`, invisible to every pinned pivot/scan count), add row→block/contact bookkeeping and bridge provenance maps (`PieceOfBlock`/`ConnectionOfJoint`), verify it fail-closed with a Farkas check (`yᵀA≤tol`, `yᵀb>0`, non-empty), and **canonicalize the non-unique degenerate dual** into a stable named set (normalize the certificate, per-block relative-magnitude threshold τ, minimal-support lexicographic tie-break). Driven by an oracle-level mechanism-correctness test against an INDEPENDENT kinematic oracle **plus a permuted-column determinism assertion** (the analog of the LU/eta fuzz — build it first) and Farkas fail-closed mutations. The slow sweep must stay **bit-identical**. τ and the tie-break are a "ruling wearing a constant's clothes" — pick them against the permutation fuzz, never by eye.
- **THE GATE ON THE SLICE:** the break set now depends on the LP's last bits in every build, so before 3b wires any wall to it, the canonical mechanism must be proven (i) reproducible under column permutation and (ii) stable across a **Shipping-FP build** (R7 — build the module Shipping, compare the named mechanism on a clean-collapse, a near-boundary-degenerate, and a row-20-style-local fixture). A wobbling break set fails "destruction you can reason about"; if it can't be pinned, the slice stops here and the design changes shape.
- **3b — release-rule wiring:** below the cap the strength sweep is DEMOTED to the estimator it already is (computes utilisations for the overlay, does NOT latch breaks); the mechanism is the sole break authority — a piece is released iff the mechanism moves it, a joint severed iff it opens/slides. This greens rows 10/19 (empty mechanism → nothing dropped, via making `GetPieceSupport` LP-authoritative below the cap — a ruling: a piece in the LP force system reads Supported) and the dry stack (per-joint veto — NOT the whole-structure veto D6 forbade, so no row-20 over-protection). Drops Slice 2's flood + remainder second-solve (the mechanism subsumes the over-release bug it patched — an innocent body has a zero dual triple). `StrandsToday 3/6 → 0` is a re-pin whose MEANING changes (router-stranded → LP-carried).
- **Row 20 is NOT a 3b green.** The global feasibility LP is feasible (λ\*=82.63) so its mechanism is empty → 0 teeth drop, flipping row 20 from today's over-count (9) to an under-count (0), still red the other way. A genuine 2-tooth local mechanism needs per-region interrogation (§3.5), not the global solve. Row 20's count is 0, and re-ruling it (to Stands accepting the LP's charity, or a named inverted red like row 21) is a **Slice-4 orchestrator ruling** with the broad Class-A re-measurement — never a promised green.

**D8 — the block cap becomes the AUTHORITY-SCOPE boundary (raised 84 → 200), decoupled from the strict latency band.** *(Ruled 2026-08-27 by the orchestrator when 3b landed; open to veto.)* 3b needs rows 10/19 (150/125 blocks) in LP scope to fix them in PRODUCTION, and the choice is binary: either the production cap covers them (they are actually fixed) or it does not (they stay router-wrong — a green-test-over-broken-production lie the project forbids). So the cap is raised to 200 and reframed as the *authority scope* (the LP governs human-scale structures; the ~1,220-block flagship and the giant corbels stay on the router), **no longer the strict ~84–104 per-action latency band** the `DeletionResolveLatencyLadder` measured. *The latency cost is real and logged as debt:* a structure in the 104–200-block band that actually collapses costs ~0.5–1 s per feasibility solve × several cascade passes; the pivot-based §5.6 budget does NOT catch it (wall-clock grows ×174 vs pivots ×11.9 — the ladder's own finding), so the block cap was the only latency guard and 200 removes it for that band. **Accepted because no production structure sits in 84–200 today** (fixtures are small, the flagship is ~1,220), so no player action is affected now; if a mid-size structure is ever built, this needs a wall-clock-aware fail-close or an async solve or a lower cap. Rationale: for a believability-first destruction game, correct-but-occasionally-laggy on a structure the player built beats fast-but-wrong.

**D8-fallout — the Slice-4 verdict rulings (all CONSERVATIVE — the LP is more correct than the deleted heuristics; each re-pinned predictions-first, never scaled).** Every verdict that moved below the cap moved in the direction of the LP holding pieces up in agreement with the oracle: **rows 10/19 STAND** (λ\* 111.5 / 48.0 — the router wrongly stranded them); **row 20 STANDS** (LP λ\* **218.42** — the authoritative `WallsAndLadders` sweep pin; the "82.63" first written here was a stale prose figure, corrected 2026-08-27 — the LP holds the teeth ~129× clear, a margin independent of λ\*; re-ruled from LocalLoss, the old heuristic's 2-tooth drop was its error); the **17-brick hang STANDS** (`MomentBreaksTheJointItOverloads` — the LP sees the redistribution the local 1.0096 per-joint check cannot); the **three beams STAND** and the **one-cell dry seat STANDS** (production now agrees with the oracle it used to dissent from); the **132-block dry ragged wall** holds its bulk (`World.Push` released 4 not 111). Stale `DropsToday`/`StrandsToday`/caption anchors and the `EveryLevelsCaptionTellsTheTruth` tripwire are re-pinned to the new correct behaviour. Row 21 stays a ruled-Collapse inverted red (LP stands 17.24, unchanged). Nothing moved in the fail-open direction; the `MustNotFall`/`Locality` guards stayed green.
