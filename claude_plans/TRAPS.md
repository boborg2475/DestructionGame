...
| — | case-21 survivor region widened (`{0,3,-1.0,1.75}` → `{0,4,…}`) | exactly 1 — "COLLAPSE must not take the masonry beside it; 2 of 22 named survivor(s) came down, {c4/1, c4/0}"; the revived Collapse arm's bite-prover |
| — | `ThrustUu` scaled to zero (production) | case 21 STANDS entirely (passes 0, worst 0.736696) and all 35 MustFall bricks fire — its collapse IS the arch-thrust refusal; case 22 untouched at 254, so the two Collapse rows fail by different routes |
