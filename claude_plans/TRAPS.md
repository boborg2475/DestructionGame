...
| — | `ReseatSpannedGroups` early-return (spell `\|\| Pieces.Num() > INT32_MIN` — bare return trips C4702) | coarse (removes a routing mechanism): **38 errors** (re-measured 2026-08-12 after the 9/10/19 re-rulings; the old 36 predates them) across the wall catalogue, span test, caption test and preconditions; the sharp half survives verbatim: case 8 drops exactly the two bricks the retired LOCAL LOSS named ({c4/5, c4/6}) — production's case-8 STANDS is spanned-group re-seating, proven |
