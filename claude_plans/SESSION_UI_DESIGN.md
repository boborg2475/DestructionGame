...
| **S4** | **The strip on screen** | `ADestructionGamePlayerController::BuildSessionToolbarPanel() -> TSharedRef<SWidget>`, `GetSessionToolbarState()`, `OnToolbarButton(EToolbarButtonId)` | `World.Session.ToolbarDrivesTheSession` — headless: clicking through `OnToolbarButton` moves the state *and* pushes it onto `UBuildModeComponent` (`CurrentMaterial`, `CurrentExtentCm`, `BuildPlaneZCm` from `CoursePlaneZCm`, `bBuildGrounded` from `IsCourseGrounded`) | Build tab lit with its accent bar, Brick lit, Snap lit, `Course 0`, `Clear build` visibly greyed |
